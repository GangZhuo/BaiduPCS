            ...
            "items": [{ 
              ...
              "method": "backup", /*执行的操作。
                                     ...
                                     backup  - 备份 /var/www 到 /backup/www。
                                               ...
                                               如果网盘文件或目录在本地文件系统中不存在，则会删除网盘文件或目录到本地。
                                     restore - 还原 /backup/www 到 /var/www，
                                               ...
                                               未启用MD5时，如果网盘文件备份时间大于本地文件的修改时间，将还原文件到本地文件系统，
                                               此时网盘文件被覆盖，否则跳过该文件。
                                               如果本地是文件，但是网盘是目录，则会删除本地文件后再还原网盘目录到本地文件系统，
                                               如果本地是目录，但是网盘是文件，则会删除本地目录后再还原文件到本地文件系统。
                                     ...
                                     combin  - 合并/var/www 和 /backup/www。
                                               ...
                                               网盘文件备份时间，则备份该文件到网盘，如果本地文件修改时间小于网盘文件备份时间时，
                                               则还原网盘文件到本地，否则跳过文件。
...
### 比较本地文件或目录<local path>和网盘文件或目录<remote path>的异同
    ...
    如果本地文件或目录在网盘中不存在，则任务需备份本地文件或目录到网盘。
